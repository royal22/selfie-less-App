...
def	timeout():
	...
	if (int((cnt_request - prev_cnt_req) /20) + 1) < cnt_active_cont:  #scale down (cnt_request/20) + 1
			...
			for l in range(8000 + cnt_active_cont,8000 + int(diff) + cnt_active_cont):
				os.system("sudo docker run -d -p {}:80 -v /home/hduser/Documents/assignment4/database1:/data acts".format(l))
				time.sleep(1)
...
while(True):
	...
	if cnt_request == 0 and i==1:   #if cnt_request doesnot increase then we dont need to again start the timer
		timer.start()
		i=0
